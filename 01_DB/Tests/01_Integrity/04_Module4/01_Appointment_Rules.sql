-- =========================================================
-- INTEGRITY TEST — MODULE 4 — APPOINTMENT RULES
-- =========================================================
--
-- TYPE:     01_Integrity (executable assertions via RAISE NOTICE)
-- REQUIRES: 04_Loaders/03_TestData.sql (or equivalent fixture chain)
-- RUN:
--   psql -v ON_ERROR_STOP=1 -f 01_Integrity/04_Module4/01_Appointment_Rules.sql
-- =========================================================
...
begin
    raise notice '=========================================================';
    raise notice 'MODULE 4 — INTEGRITY: APPOINTMENT RULES';
    raise notice '=========================================================';
    ...
    if v_cli is null then
        raise notice 'SKIP: no active ownership / expert — run 04_Loaders/03_TestData.sql first.';
        return;
    end if;
    ...
    begin
        call sp_create_appointment(v_cli, v_ani, v_vet, v_spe, (now() + interval '3 days')::timestamp);
    ...
    begin
        call sp_create_appointment(v_cli, v_ani, v_vet, v_spe, (now() - interval '1 day')::timestamp);
    ...
    if v_bad_spe is not null then
        begin
            call sp_create_appointment(v_cli, v_ani, v_vet, v_bad_spe, (now() + interval '8 days')::timestamp);
    ...
    raise notice '=========================================================';
    raise notice 'MODULE 4 — INTEGRITY FINISHED';
    raise notice '=========================================================';
end;
$$;
